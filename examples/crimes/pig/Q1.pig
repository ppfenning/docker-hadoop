...
-- get needed columns
crimes = FOREACH crimes GENERATE primary_type;
-- problem
x = GROUP crimes BY primary_type;
x = FOREACH x GENERATE group as primary_type, COUNT($1) as cnt;
x = ORDER x BY cnt DESC;
x = LIMIT x 1;
STORE x INTO 'hdfs://namenode:9000/pig-out/Q1/' USING PigStorage('\t','-schema');
